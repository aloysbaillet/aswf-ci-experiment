...
class QtConan(ConanFile):
    ...
    no_copy_source = True
    default_options = ("shared=True", "commercial=False", "opengl=desktop", "openssl=True", "GUI=True", "widgets=True", "config=None") + tuple(module + "=True" for module in submodules)
    short_paths = True
    ...
    def source(self):
        base = "Qt561ForMaya2018Update4.zip"
        if os.path.exists(base):
            self.output.info("Found local source tarball {}".format(base))
            tools.unzip(base, keep_permissions=True)
        else:
            url = "https://www.autodesk.com/content/dam/autodesk/www/Company/files/2018/" + base
            self.output.warn("Downloading source tarball {}".format(url))
            tools.get(url, keep_permissions=True)
        shutil.move("qt-adsk-5.6.1-vfx", "qt5")
        tools.unzip("qt561-webkit.tgz", destination="qt5", keep_permissions=True)
